 ...
 */
public class DisplayBackground extends AsyncTask<Object, String, Appointment> {
...
public DisplayBackground(Context context, Patient p1) {
...
protected Appointment doInBackground(Object... arg0) {
        try {
            String link = "http://acecoders.netau.net/viewENT.php";
            ...
            if (res.length()>0){
                ...
                for(int i=0;i<14; i++)
                    arr[i] = "";
                arr = res.split("<br>",-1);
                appt  = new Appointment("ENT",arr[0],arr[1],arr[2],arr[3],arr[4],arr[5],arr[6]);
